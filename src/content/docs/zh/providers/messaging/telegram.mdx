...
setup 成功后，Telegram pack 会通过 `greentic.setup.actions.v1` 暴露一个通用 setup 操作：
...
| 必需 setup output | `bot_username` |

setup host 可以将其渲染为按钮、显示 URL，并提供 HTML 片段。该链接会在 Telegram 中打开 bot，让用户开始对话。
