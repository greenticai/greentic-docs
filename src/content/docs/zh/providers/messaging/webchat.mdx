...
4. **Embed in Website**
   ...
   <Aside type="tip">
   When the `greentic.cap.webchat.embed.v1` capability is enabled, embed snippets are auto-generated in `assets/webchat-gui/embed/{tenant}.html`. See the [WebChat Embedding guide](/providers/messaging/webchat-embedding/) for full configuration reference and the public JavaScript API.
   </Aside>
...
| `public_base_url` | Yes | Public runtime URL used in generated links and webhook/client configuration |
| `base_url` | Yes | Base URL used by the GUI assets and generated embed snippets |
| `route` | No | Web route, usually `webchat` |
| `mode` | No | WebChat mode, for example `local_queue` in local demos |
| `tenant_channel_id` | No | Tenant/channel binding used by the generated GUI |
| `jwt_signing_key` | Yes for GUI auth/token mode | Signing key for WebChat-issued tokens |
...
<Aside type="note">
Use `gtc setup ./my-bundle` or `gtc wizard --schema` to confirm the exact fields for your installed WebChat pack. The demo answers files use `messaging-webchat-gui` for the hosted UI and embed snippets.
</Aside>
...
<Aside type="tip">
For the full capability system and scaffold details, see [Bundle Assets](/concepts/bundle-assets/). For embed types and the chat bubble configuration reference, see [WebChat Embedding](/providers/messaging/webchat-embedding/).
</Aside>
...
| Provider | Wizard fields | Notes |
|----------|---------------|-------|
| Google | `oauth_enable_google`, `oauth_google_client_id`, `oauth_google_client_secret` | Google OAuth 2.0 |
...
- [WebChat Embedding](/providers/messaging/webchat-embedding/) - Embed types, chat bubble config, and public JS API
- [Bundle Assets](/concepts/bundle-assets/) - Capability system and skin overlay mechanism
- [Adaptive Cards](/components/adaptive-cards/) - Create rich cards
- [Components](/concepts/components/) - Customize component-driven behavior
