...
当前 `messaging-teams` setup 路径使用 provider-owned setup wizard 和 backend contract。它会引导 Microsoft device-code sign-in，创建或复用 Bot Framework app identity，注册当前 Greentic endpoint，发布 Teams app，为 setup user 安装，并暴露通用 **Add to Teams** 操作。请保持 `public_base_url` 与当前 tunnel 或生产 URL 一致；当该 URL 改变时重新运行 setup。

在 answers 文件中，提供公共 URL 和可选的命名/tenant 提示：
...
已注册的 Bot Framework messaging endpoint 必须指向当前公共 runtime URL。如果 `gtc start` 创建了新的 Cloudflare Tunnel 或 ngrok URL，请在使用 **Add to Teams** 或测试 bot 之前重新运行 setup。
...
| 必需 setup output | `add_to_teams_url` |

setup host 可以在 setup 返回 `setup_status.ok: true` 后，将其渲染为按钮、显示 URL，并提供 HTML 片段。
