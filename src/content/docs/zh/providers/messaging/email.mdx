...
`messaging-email` 是 SMTP 出站提供商。它不会轮询 IMAP 或 POP3 邮箱。入站邮件通常通过事件提供商或提供商 webhook 处理，例如 SendGrid Inbound Parse、Mailgun Routes、AWS SES，或在已安装 pack 支持时通过 Microsoft Graph subscriptions 处理。
