...
在一个 bundle 中演示所有 event providers：webhooks、cron timers、通过 SendGrid 发送 transactional email，以及通过 Twilio 发送 SMS。

- **Channels:** WebChat GUI
- **External services:** SendGrid（可选）、Twilio（可选）、incoming webhook
...
这会关闭 runner，以及 `gtc start` 启动的所有辅助进程（NATS、tunnels）。
