...
|------|------|
| `greentic.toml` | 运行时配置 |
| `greentic.demo.yaml` | Bundle 配置 |
| `answers.json` | Provider setup 答案 |
...
| 变量 | 配置路径 | 说明 |
|------|----------|------|
| `GREENTIC_HOST` | `server.host` | 绑定地址 |
| `GREENTIC_PORT` | `server.port` | HTTP 端口 |
| `GREENTIC_LOG_LEVEL` | `logging.level` | 日志级别 |
| `GREENTIC_LOG_FORMAT` | `logging.format` | 日志格式 |
| `GREENTIC_NATS_URL` | `nats.url` | NATS 服务器 URL |
| `GREENTIC_REDIS_URL` | `redis.url` | Redis URL |
| `GREENTIC_OTLP_ENDPOINT` | `telemetry.otlp_endpoint` | OTLP 端点 |

## 配置优先级

1. 环境变量（最高）
2. 配置文件（`greentic.toml`）
3. 默认值（最低）

## 下一步

- [gtc start](/zh/cli/start/)
- [gtc setup](/zh/cli/setup/)
