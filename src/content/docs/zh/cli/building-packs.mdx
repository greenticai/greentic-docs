---
title: 构建 Packs
description: 创建、构建并管理 Greentic packs
---
...
4. **添加 components**（如果有）
   ...
   构建 WASM components 并放入 `components/`：
...
<Aside type="caution">
分发前务必为 packs 签名。在生产环境中，未签名的 packs 可能会被运行时拒绝。
</Aside>
...
1. **使用语义化版本** - 使用 semver（MAJOR.MINOR.PATCH）
2. **为所有发布版本签名** - 绝不要分发未签名的 packs
3. **包含 SBOM** - 为安全审计记录依赖
4. **发布前测试** - 验证 flows 并测试 components
5. **完整编写文档** - 在 pack 中包含 README
6. **让 packs 保持聚焦** - 每个 pack 只负责一个功能或 provider
7. **使用 CI/CD** - 自动化构建和发布
...
- [Pack Format Reference](/zh/reference/pack-format/) - 完整规范
- [Flow Schema Reference](/zh/reference/flow-schema/) - YAML schema
- [Components Guide](/zh/concepts/components/) - 构建 WASM components
