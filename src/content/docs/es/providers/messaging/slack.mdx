...
El pack de Slack declara una acción de setup genérica:

| Campo | Valor |
|-------|-------|
| Etiqueta | `Add to Slack` |
| Tipo | `deep_link` |
| Plantilla de URL | `{slack_add_url}` |
| Salida de setup requerida | `slack_add_url` |

Los hosts de setup pueden renderizarla como botón, mostrar la URL y proporcionar un fragmento HTML después de que setup devuelva `setup_status.ok: true`. Mantén esta acción posterior al setup separada de **Setup Slack App**, que es la acción específica del proveedor que crea o actualiza la configuración de la app.
