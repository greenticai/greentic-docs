...
- Public web and provider routes declared by installed packs, including WebChat assets, provider ingress paths, and provider setup routes when available
...
When a tunnel URL changes, re-run `gtc setup` for providers that register external callbacks or app endpoints. Teams, Slack, Telegram, Webex, WhatsApp, Microsoft Graph subscriptions, and WebChat links may all contain the previous public URL in provider-side configuration or generated setup output.
...
For the current Teams provider, setup also registers the Bot Framework-compatible endpoint for the active public URL. If you restart with a different Cloudflare Tunnel or ngrok URL, run setup again before using the **Add to Teams** link or testing the bot.
...
| `POST /v1/messaging/ingress/{provider}/{tenant}/{team}` | Current provider ingress route used by messaging packs |
| `GET /v1/web/{pack}/{tenant}/...` | Pack-declared public web assets such as WebChat GUI and provider setup modules |
| `GET/POST /v1/messaging/setup/{provider}/{tenant}/...` | Provider setup routes when declared by the pack, such as Teams setup state and actions |
