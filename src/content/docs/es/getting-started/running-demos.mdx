...
Ejercita todos los proveedores de eventos en un solo bundle: webhooks, temporizadores cron, correo transaccional mediante SendGrid y SMS mediante Twilio.

- **Canales:** WebChat GUI
- **Servicios externos:** SendGrid (opcional), Twilio (opcional), webhook entrante
...
Flujo de creación de incidentes con adaptive cards que apunta a WebChat y Webex, y luego abre un incidente en Better Stack.

- **Canales:** WebChat GUI, Webex
- **Servicios externos:** Better Stack (`betterstack_token` en setup-answers)
- **Demo ID:** `incident`
...
Esto apaga el runner y cualquier proceso auxiliar (NATS, túneles) que `gtc start` haya levantado.
