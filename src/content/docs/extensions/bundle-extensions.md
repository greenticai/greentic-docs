...
- `extension.wasm` — the compiled component that implements the frozen `greentic:extension-bundle@0.1.0` WIT contract
...
A new reference bundle extension lives under `reference-extensions/<name>/` in the [`greentic-bundle-extensions`](https://github.com/greentic-biz/greentic-bundle-extensions) repository. The shape:

```
reference-extensions/<name>/
├── Cargo.toml              [package.metadata.component] + wit-bindgen deps
...
├── src/lib.rs              WIT Guest impls for manifest/lifecycle/recipes/bundling
