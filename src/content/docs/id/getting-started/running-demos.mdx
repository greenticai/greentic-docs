...
Menguji setiap provider event dalam satu bundle: webhook, cron timer, transactional email melalui SendGrid, dan SMS melalui Twilio.

- **Channel:** WebChat GUI
- **Layanan eksternal:** SendGrid (opsional), Twilio (opsional), incoming webhook
- **ID demo:** `quickstart-event`
...
Flow pembuatan incident berbasis adaptive card yang menargetkan WebChat dan Webex, lalu membuka incident di Better Stack.

- **Channel:** WebChat GUI, Webex
- **Layanan eksternal:** Better Stack (`betterstack_token` di setup-answers)
- **ID demo:** `incident`
...
Ini mematikan runner dan proses tambahan apa pun (NATS, tunnel) yang dijalankan oleh `gtc start`.
