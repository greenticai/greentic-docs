...
## Model setup Teams saat ini

Jalur `messaging-teams` saat ini menggunakan wizard setup milik provider dan kontrak backend. Wizard ini memandu sign-in Microsoft device-code, membuat atau menggunakan ulang identitas app Bot Framework, mendaftarkan endpoint Greentic aktif, menerbitkan app Teams, menginstalnya untuk user setup, dan mengekspos aksi generik **Add to Teams**. Jaga `public_base_url` tetap sesuai dengan tunnel aktif atau URL produksi; jalankan ulang setup saat URL berubah.

Dalam file jawaban, berikan URL publik dan petunjuk nama/tenant opsional:
...
Endpoint messaging Bot Framework yang terdaftar harus menunjuk ke URL runtime publik yang aktif. Jika `gtc start` membuat Cloudflare Tunnel atau URL ngrok baru, jalankan ulang setup sebelum menggunakan **Add to Teams** atau menguji bot.
...
| Output setup wajib | `add_to_teams_url` |

Host setup dapat merender ini sebagai tombol, menampilkan URL, dan menyediakan snippet HTML setelah setup mengembalikan `setup_status.ok: true`.
