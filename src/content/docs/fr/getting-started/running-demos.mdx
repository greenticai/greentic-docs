...
Exercises every event provider in one bundle: webhooks, cron timers, transactional email via SendGrid, and SMS via Twilio.

- **Channels:** WebChat GUI
- **External services:** SendGrid (optional), Twilio (optional), incoming webhook
...
A lead-capture and contact-routing digital worker for Greentic AI, wired for multiple messaging channels so the same flow can run across customer-facing entry points.

- **Channels:** Slack, Teams, WebChat GUI, Webex, Telegram, WhatsApp, Email
- **External services:** Messaging provider credentials for the channels you enable
- **Demo ID:** `greentic-ai`

```bash
gtc wizard --answers https://github.com/greenticai/greentic-demo/releases/latest/download/greentic-ai-create-answers.json
gtc setup ./greentic-ai-demo-bundle --answers https://github.com/greenticai/greentic-demo/releases/latest/download/greentic-ai-setup-answers.json
gtc start ./greentic-ai-demo-bundle
```
...
Adaptive-card incident creation flow that targets WebChat and Webex, then opens an incident on Better Stack.

- **Channels:** WebChat GUI, Webex
- **External services:** Better Stack (`betterstack_token` in setup-answers)
- **Demo ID:** `incident`
...
This shuts down the runner and any auxiliary processes (NATS, tunnels) that `gtc start` brought up.
