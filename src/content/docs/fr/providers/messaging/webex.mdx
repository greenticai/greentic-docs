...
Une fois setup reussi, le pack Webex expose une action de setup generique via `greentic.setup.actions.v1` :

| Champ | Valeur |
|-------|--------|
| Label | `Add to WebEx` |
| Kind | `deep_link` |
| Modele d URL | `https://web.webex.com/teams/messages/new?email={bot_email}` |
| Sortie setup requise | `bot_email` |

Les hosts de setup peuvent l afficher comme bouton, montrer l URL et fournir un snippet HTML. Le lien ouvre Webex avec un nouveau message adresse au bot.
