...
<Aside type="note" title="gtdx kinds">
  `gtdx new --kind` accepts `design`, `bundle`, `deploy`, `provider`, `wasm-component`, `mcp`, `llm`, and `addon`. Use `--kind mcp` for a `wasix:mcp/router` local-wasm tool. See [gtdx CLI](/extensions/gtdx-cli/).
</Aside>
...
`gtdx` shells out to `greentic-mcp-gen` and auto-authors `describe.json` — deriving
`runtime.permissions.network` from the spec's `servers` and `secret_requirements` from its
security schemes. See [Creating MCP Tools](/mcp/creating-tools/) for the full step-by-step
