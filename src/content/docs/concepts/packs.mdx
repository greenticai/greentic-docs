...
| **Extension packs** | `.gtpack` | Capability and platform extensions: messaging, events, OAuth, state, secrets, telemetry, deployers, control, observer hooks, contracts, and ops metadata. |
...
<Aside type="tip">
The extension mechanism does not mean every extension is loaded the same way. Runtime capability packs, deployer packs, static-route packs, and integration surfaces can have different consumers, but they share the same idea: a signed pack declares a stable extension surface and the tooling can validate and replay it.
</Aside>
...
1. Choose the extension type in `gtc wizard`.
...
6. Run `gtc dev pack doctor <PACK_DIR>` and build/sign the pack.
    ...
    inline:
      ...
      offers:
        - offer_id: state.memory.kv.01
          ...
          version: v1
          provider:
            component_ref: memory-extension
            op: state.dispatch
          priority: 100
...
```bash
gtc dev pack lint --in ./my-pack
gtc dev pack resolve --in ./my-pack
gtc dev pack build --in ./my-pack
gtc dev pack doctor ./my-pack
```
...
6. Validate with `gtc dev pack doctor` before every build.
