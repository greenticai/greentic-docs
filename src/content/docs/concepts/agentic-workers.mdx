...
<Aside type="tip" title="Declare each tool's argument schema">
The runtime resolves every allow-listed tool against its extension at call time and passes
the tool's declared `input_schema` to the model as the function parameters. Declare that
schema in the extension's `describe.json` — a tool that ships without one reaches the model
with an empty parameter object, and you are then forced to spell its arguments out in the
system prompt.
</Aside>
...
[`component-tavily-ext`](/extensions/writing-extensions/) and replace the tool logic.

Tools are declared in the extension's `describe.json`, and each one must declare the
`agentic_worker` capability to be usable by a worker:
...
<Aside type="caution" title="A capabilities list in Rust is ignored">
For `apiVersion: greentic.ai/v2` the tool list is read from `describe.json` only — the
component's `list-tools` export is never called. A `ToolMeta { capabilities: … }` built in
`src/lib.rs` has no effect. Note also that omitting `capabilities` does **not** mean "all
surfaces": it defaults to `["flow"]`, which makes the tool invisible to every agentic
worker. See [Extension Tools and Node Types](/extensions/extension-tools/).
</Aside>
