...
| `greentic.cap.webchat.i18n.v1` | Multi-language WebChat | Enables locale picker and scaffolds locale JSON files under `./assets/`. When a worker is exported with additional languages, `greentic-pack` automatically translates the source strings and packs the locale files inside the application `.gtpack`, so the picker shows exactly the languages selected at export without any manual copy step. |
