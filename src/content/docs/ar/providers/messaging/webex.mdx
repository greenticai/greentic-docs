...
بعد نجاح setup، يعرض pack Webex إجراء setup عاما عبر `greentic.setup.actions.v1`:

| الحقل | القيمة |
|-------|--------|
| Label | `Add to WebEx` |
| Kind | `deep_link` |
| قالب URL | `https://web.webex.com/teams/messages/new?email={bot_email}` |
| خرج setup المطلوب | `bot_email` |

يمكن لمضيفي setup عرضه كزر، وإظهار URL، وتوفير HTML snippet. يفتح الرابط Webex مع رسالة جديدة موجهة إلى bot.
