...
The flow-level field is the single source of truth — Fast2Flow scoping, design-time tooling, and Phase-D runtime wiring all read from it.

<Aside type="note">
Today the same definitions are also inlined into the slot-extractor node payload so the pack runs end-to-end on the current runtime. Phase D removes the duplication by reading the flow-level field directly and passing it into the extractor call.
</Aside>
