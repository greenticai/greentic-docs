...
2. **Read the entities downstream.** `greentic-start` reads the `entities` array and surfaces them to card templates as `prefill_<kind>` substitution keys (e.g. `prefill_person`, `prefill_date`, `prefill_date_iso`), so a check-in card opens with the pet name and arrival time already filled in.
3. **Use `formats.iso` for date inputs.** Bind Adaptive `Input.Date` fields to the `iso` form, not the compact `normalized`.

<Aside type="caution">
Prefill entities travel on the **native routing-host** path. The WebAssembly (WIT) entrypoint has no vector type for entities yet, so it drops them — a flow dispatched through the wasm component receives the `Dispatch` target but an empty `entities` array. Use the [`greentic-fast2flow-routing-host`](#cli--tooling) binary when you need prefill end-to-end.
</Aside>
