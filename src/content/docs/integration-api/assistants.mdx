...
`reason` is a machine token — `missing_secrets_broker` for `mcp:` tools, `missing_sorx_url` for `sorla:` tools. Branch on the entry being present rather than on the token itself. The worker will still run — those tools simply will not be offered to the model. See [Agentic-worker MCP tools](/integration-api/behaviour/#mcp-tools-on-agentic-workers).
