...
`reason` is a stable token: `missing_admin_credentials` for `mcp:` tools, `missing_sorx_url` for `sorla:` tools. The worker will still run — those tools simply will not be offered to the model. See [Agentic-worker MCP tools](/integration-api/behaviour/#mcp-tools-on-agentic-workers).

<Aside type="caution" title="The field is absent on the healthy path">
When every bound tool is resolvable, `unreachableTools` is **omitted from the response entirely**. It is not sent as an empty array.

A pipeline gate that reads `response.unreachableTools.length` therefore works on the failing publish and crashes on the successful one — the worst possible place to put the break. Default the field before you read it.
</Aside>
