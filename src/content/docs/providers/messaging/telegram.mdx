...
## Add to Telegram Action

After setup succeeds, the Telegram pack exposes a generic setup action through `greentic.setup.actions.v1`:
...
| Required setup output | `bot_username` |

Setup hosts can render this as a button, show the URL, and provide an HTML snippet. The link opens the bot in Telegram so users can start the conversation.
