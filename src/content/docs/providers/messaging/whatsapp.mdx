...
3. **Configure Webhook**
   ...
   - Set Callback URL: `https://your-domain.com/v1/messaging/ingress/messaging-whatsapp/{tenant}/{team}`
...
| `business_account_id` | Yes | WhatsApp Business Account ID |
| `api_base_url` | Yes | Graph API base URL, normally `https://graph.facebook.com` |
| `api_version` | Yes | Graph API version used by the installed pack |

<Aside type="note">
The setup layer also accepts the older `access_token` alias and stores it as `whatsapp_token`, but new answers files should use the generated key. Use `gtc setup --dry-run --emit-answers answers.json ./my-bundle` to confirm the exact fields for the pack version in your bundle.
</Aside>

## Network Access

WhatsApp Cloud API uses Meta webhooks for incoming messages and Meta Graph API calls for outgoing sends. Expose the Greentic runtime through HTTPS on port `443`; local development can use a tunnel that forwards the public URL to the local runtime port.

| Direction | Protocol and port | Purpose |
|-----------|-------------------|---------|
| Incoming | HTTPS `443` from Meta to Greentic | Webhook verification and WhatsApp message callbacks |
| Outgoing | HTTPS `443` from Greentic to `graph.facebook.com` or the configured `api_base_url` | Cloud API sends, media operations, and webhook setup/verification calls when supported |

No WhatsApp-specific TCP port needs to be opened on the Greentic host. Meta must be able to reach the configured callback URL over HTTPS and validate the verify token.
