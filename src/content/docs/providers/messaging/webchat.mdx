...
The `messaging-webchat-gui` provider includes a full web UI served at `/v1/web/webchat/{tenant}/`.

### Embed Widget

Add a floating chat bubble to any website:
...
**Public API:**
- `greenticChat.open()` / `.close()` / `.toggle()` / `.isOpen()`
