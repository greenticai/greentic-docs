...
The `greenticChat.init()` function accepts the following options:
...
When options like `title`, `primaryColor`, or `logo` are not explicitly provided in the `greenticChat.init()` call, the embed script automatically loads defaults from the tenant's `skin.json` file. This means your chat bubble inherits your brand configuration without duplicating it in the embed code.
...
1. Explicit options passed to `greenticChat.init()` (highest priority)
...
- [WebChat Provider](/greentic-docs/providers/messaging/webchat/) -- full setup, OAuth, and features
- [Bundle Assets](/greentic-docs/concepts/bundle-assets/) -- capability system and skin overlay
- [gtc wizard](/greentic-docs/cli/wizard/) -- set capabilities in the answers file
