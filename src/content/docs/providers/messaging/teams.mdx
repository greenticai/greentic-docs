...
2. **Authorize Microsoft Graph**

   Setup starts a device-code flow and asks you to authorize the Microsoft application in your browser. The default Greentic public client id is included in the pack, or you can provide your own `client_id`.

3. **Select a Team and Channel**

   Setup calls Microsoft Graph to list joined teams and channels. It persists both the identifiers and labels:
   - `team_id` and `team_name`
   - `channel_id` and `channel_name`

4. **Optionally create a desired channel**

   Provide `desired_channel_name` to use the bundle name or another label as a setup hint:
...
5. **Start or restart the runtime**
