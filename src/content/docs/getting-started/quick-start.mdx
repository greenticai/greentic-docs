...
- **Redis** if you run an [Agentic Worker](/concepts/agentic-workers/) — the `dw.agent` node uses Redis for multi-turn memory and is disabled without it (`brew services start redis`, or `docker run -d -p 6379:6379 redis:alpine`)
...
<Aside type="note">
Agentic Workers need **Redis** running for multi-turn memory — set `GREENTIC_AW_REDIS_URL=redis://127.0.0.1:6379`. Without it the `dw.agent` node is disabled. See [Agentic Workers](/concepts/agentic-workers/) for the full `AgentConfig` reference (tools, guardrails, memory, knowledge, limits).
</Aside>
