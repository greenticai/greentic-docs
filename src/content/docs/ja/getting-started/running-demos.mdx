...
1 つの bundle であらゆる event provider を試します。webhooks、cron timers、SendGrid 経由の transactional email、Twilio 経由の SMS が含まれます。

- **Channels:** WebChat GUI
- **External services:** SendGrid（任意）、Twilio（任意）、incoming webhook
...
これにより、runner と、`gtc start` が起動した補助プロセス（NATS、tunnels）が停止します。
