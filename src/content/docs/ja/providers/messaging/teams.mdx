...
## 現在の Teams setup model

現在の `messaging-teams` setup path は、provider-owned setup wizard と backend contract を使用します。Microsoft device-code sign-in を案内し、Bot Framework app identity を作成または再利用し、active Greentic endpoint を登録し、Teams app を公開し、setup user にインストールし、汎用 **Add to Teams** action を公開します。`public_base_url` は active tunnel または production URL と一致させてください。その URL が変わった場合は setup を再実行します。

answers file では public URL と任意の naming/tenant hint を指定します:
...
登録済み Bot Framework messaging endpoint は active public runtime URL を指す必要があります。`gtc start` が新しい Cloudflare Tunnel または ngrok URL を作成した場合は、**Add to Teams** を使う前、または bot をテストする前に setup を再実行してください。
...
| 必須 setup output | `add_to_teams_url` |

Setup host は、setup が `setup_status.ok: true` を返した後に、これを button として表示し、URL を見せ、HTML snippet を提供できます。
