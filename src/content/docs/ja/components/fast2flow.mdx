...
The pack can also ship its routing index inline as `assets/intent-index.json` (same shape as the `index.json` produced by `greentic-fast2flow bundle index`). If the deployer doesn't populate an external `<indexes_path>/<scope>/index.json`, the runtime's pack-fallback materializer reads the asset, writes it under the indexes path, and reuses it for the rest of the process lifetime.
...
<Aside type="note">
This section is in English in the localized docs and pending translation. Track translation work in a follow-up PR.
</Aside>
