...
<Aside type="tip">
The wizard performs **capability-based dependency resolution**. When the demo's create-answers document references a provider that declares `required_capabilities` (e.g. messaging providers needing `greentic:state/state-store`), the wizard automatically adds a satisfying pack such as `state-memory` to the generated bundle — you do not have to list it manually.
</Aside>
