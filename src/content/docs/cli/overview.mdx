...
| `gtc dev` | `greentic-dev` | Developer tooling, including pack, flow, component, MCP (e.g. `gtc dev mcp gen`), GUI, secrets, and coverage passthroughs |
...
# Generate an MCP component from an OpenAPI spec (passthrough to greentic-mcp-gen)
gtc dev mcp gen --spec ./openapi/weatherapi.yaml --output-dir ./output
...
`gtc dev mcp gen` is a passthrough to `greentic-mcp-gen`. For a publish-ready extension
with an auto-authored `describe.json`, use `gtdx new --kind mcp --from-openapi` instead.
